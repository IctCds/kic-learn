...
const Dialog = ({exam, newExam, setDialog, updateExam}) => {
  return (
    <div className='w-[300px] bg-white text-gray-800 flex flex-col py-4 px-2 rounded-md shadow-md'>
    <div>
      <p className='text-sm'>
        Are you sure you want to switch exams from <span className='font-semibold'>{exam}</span> to <span className='font-semibold'>{newExam.name}</span>?
      </p>
    </div> 
      <div className="mt-2 flex justify-end gap-5">
        ...
        <button
          ...
          onClick={()=> {setDialog(false); updateExam(newExam)}}
        >
          YES
        </button>
      </div>
